...
public class Tank extends GameObject {
    ...
    void setTankLife(int tankLife_to_set) {
        this.tankLife = tankLife_to_set;
    }
    ...
    private void checkBorder() {
        ...
        if (x >= GameWorld.FullScreenWidth - 88) {
            x = GameWorld.FullScreenWidth - 88;
        }
        if (y < 40) {
            y = 40;
        }
        if (y >= GameWorld.FullScreenHeight - 80) {
            y = GameWorld.FullScreenHeight - 80;
        }
    }
    ...
    public void collision() { //this is only called if the tank is shot by a bullet(other than its own)
        this.removeTankLife(20);

    }

    void addTankLife(int val) {
        if (tankLife + val > 100) {
            tankLife = 100;
        } else {
            tankLife += val;
    ...
    private void removeTankLife(int val) { //private access because only the tank should be able to remove from its tankLife(this is done in the Tank's collision method)
        if (tankLife - val < 0) {
            tankLife = 0; //tank died
        } else {
            tankLife -= val;
        }
    }
    ...
    public void drawImage(Graphics2D g) {
        ...
        if (this.tankLife != 0) { //NOTE: when a tank's draw image is called, the tankLife value is only zero if a player lost, because we replenish the tankLife
            // before implicitly calling the Tank drawImage
